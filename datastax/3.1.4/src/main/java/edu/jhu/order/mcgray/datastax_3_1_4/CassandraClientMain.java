 ...
 */
package edu.jhu.order.mcgray.datastax_3_1_4;

import edu.jhu.order.mcgray.common.api.ClientStubFactory;
import edu.jhu.order.mcgray.common.api.ModelCheckerRemote;
import edu.jhu.order.mcgray.common.api.ModelCheckerRemote.ClientFeedback;
...
public class CassandraClientMain {
    ...
    private static ModelCheckerRemote stub = null;
    ...
    private static void reply(final String result, final long nano) {
        ...
        if (stub != null) {
            try {
                ...
                LOG.error("Fail to send the request result to modelchecker server", e);
                System.exit(0);
            }
        }
    }
    ...
    public static void main(final String[] args) {
        ...
        if (args.length == 1) {
            ...
            stub = ClientStubFactory.getModelCheckerStub(1099);
            if (stub == null) {
                LOG.error("Failed to get a client for model checker server");
            } else {
                try {
                    final String[] command = stub.registerClient(clientId, pid);
                    run(command);
                } catch (final RemoteException e) {
                    LOG.error("Failed to register with the model checker server");
                }
            }
        } else {
            run(args);
